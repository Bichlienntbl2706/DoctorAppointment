...
const updateReview = async ( id: string, payload: Partial<IReview> ): Promise<IReview> => {
  try {
    ...
    if (review.isUpdated) {
      throw new Error(`Review has already been updated`);
    }

    const updatedReview = await Review.findByIdAndUpdate(id, { ...payload, isUpdated: true }, { new: true });

    if (!updatedReview) {
      throw new Error(`Review cannot be updated`);
    }
