...
const Schedule = () => {
    ...
    const remove = async (id) => {
        if (currentDay !== 'wednesday') {
            message.error('Time slots can only be deleted on Wednesday.');
            return;
        }
        ...
        <>
            {data?.map((item, index) => (
                <div key={index} className="d-flex justify-content-between align-items-center">
                    ...
                    <Space size={[0, 'small']} wrap>
                        {item?.timeSlot && item?.timeSlot.map((time, index) => (
                            <Tag bordered={false}  closable={currentDay === 'wednesday'} color="processing" onClose={() => remove(time._id)} key={index + 2}>
                                {time?.startTime} - {time?.endTime}
                            </Tag>
                        ))}
                    </Space>
